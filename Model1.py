...
if verify_code:
    ...
    while current_layer < n_layers:
        print('Insert radius and average density of next layer')
...
plt.gca().set_ylim(bottom=0)
plt.tight_layout()
plt.savefig('./plot/layered_model.png')
plt.show()

if user_input or not verify_code:
    print('Computation is ended, data are saved in:', path_data, '\nplot are saved in:', path_plot)
